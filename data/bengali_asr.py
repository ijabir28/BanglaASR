...
parser = argparse.ArgumentParser(description='Processes and downloads Bangla_ASR dataset.')
parser = add_data_opts(parser)
parser.add_argument("--target-dir", default='Bangla_ASR_dataset/', type=str, help="Directory to store the dataset.")
...
BANGLA_ASR_URLS = {
    ...
    "test": ["https://www.openslr.org/resources/53/asr_bengali_f.zip"],
}
...
def main():
    ...
    for split_type, lst_bangla_asr_urls in BANGLA_ASR_URLS.items():
        ...
        transcript_file = os.path.join('Bangla_ASR_dataset', 'utt_spk_text.tsv')
        ...
        if split_type == 'train':  # Prune to min/max duration
            create_manifest(
                data_path=split_dir,
                output_name='bangla_asr_' + split_type + '_manifest.json',
